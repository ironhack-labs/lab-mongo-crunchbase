        ...
        rl.question('Type an option: ', (option) => {
          switch(option){
            ...
            db.collection('companies').find({}, {name: 1, _id: 0}).toArray((error, result) => {
              ...
              break
            case "2":
                db.collection('companies').find({}, {_id: 1}).toArray((error, result) => {
                    if (error) {
                      console.log(error)
                      rl.question(`\nType enter to continue: `, (answer) => { mainMenu() })
                    } else {
              ...
              break
              case "3":
                db.collection('companies').find({founded_year: 2004}, {_id: 1}).toArray((error, result) => {
                    if (error) {
                      console.log(error)
                      rl.question(`\nType enter to continue: `, (answer) => { mainMenu() })
                    } else {
              ...
              break
              case "5":
                db.collection('companies').find({$and: [{founded_year: 2004}, {founded_month: {$gte: 4}}, {founded_month: 6}]}, {name: 1,_id: 0}).toArray((error, result) => {
                    if (error) {
                      console.log(error)
                      rl.question(`\nType enter to continue: `, (answer) => { mainMenu() })
                    } else {
                        // no he sabido ordenarlo por fecha
