   ...
   function mainMenu(){
    ...
    rl.question('Type an option: ', (option) => {
      switch(option){
        ...
        break;
        case "5":
        db.collection('companies').find({"founded_year": 2004, "founded_month": {$gte:4, $lte:6}}, {name: 1, _id: 0, founded_month: 1}).sort({"founded_month": 1, "founded_day": 1}).toArray((error, result) => {
            if (error) {
            console.log(error);
            rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
            } else {
        ...
        db.collection('companies').find({offices:{$elemMatch: {city: "Barcelona"}}}, {name: 1, _id: 0}).toArray((error, result) => {
          ...
          break;
          case "7":
          db.collection('companies').find({},{name: 1, _id: 0}).limit(10).sort({"number_of_employees": -1}).toArray((error, result) => {
              if (error) {
              console.log(error);
              rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
          ...
          db.collection('companies').find({name:"Facebook"}, {name: 1, _id: 0}).toArray((error, result) => {
              if (error) {
                ...
                console.log(`la compañia Facebook fue encontrada:`);
                console.log(result);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              }
            })
        ...
        db.collection('companies').find({name:"Facebook"}, {number_of_employees: 1, _id: 0}).toArray((error, result) => {
            ...
            case "11":
             ...
             break;
             case "16":
             db.collection('companies').find({"tag_list":{$regex : ".*social-networking.*"}, "founded_year": {$gte:2002, $lte:2016}}, {"name":1, _id:0}).toArray((error, result) => {
               if (error) {
                 console.log(error);
                 rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
               } else {
             ...
             db.collection('companies').find({offices:{$elemMatch: {city: "London"}}}, {name: 1, "offices": 1, _id: 0}).toArray((error, result) => {
               ...
               break;
               case "18":
               db.collection('companies').find({"tag_list":{$regex : ".*social-networking.*"}, "founded_year": {$gte:2002, $lte:2016},offices:{$elemMatch: {city: "New York"}}}, {"name":1, _id:0}).toArray((error, result) => {
                 if (error) {
                   console.log(error);
                   rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                 } else {
