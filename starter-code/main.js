      ...
      rl.question('Type an option: ', (option) => {
        switch(option){
          ...
          db.collection('companies').find({founded_year:2004,founded_month:2}, {name: 1, _id: 0}).toArray((error, result) => {
            ...
            break;
            case "5":
          db.collection('companies').find({founded_year:2004,$or:[{founded_month:4},{founded_month:5},{founded_month:6}]}, {name: 1,founded_month:1, _id: 0}).sort({founded_month:1}).toArray((error, result) => {
            if (error) {
              console.log(error);
              rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
            } else {
          ...
          db.collection('companies').find({name:"Facebook"},{"products.name":1, _id: 0}).toArray((error, result) => {
            if (error) {
              ...
              result[0].products.forEach((element)=>console.log(element));
              rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
            }
          })
            break;
