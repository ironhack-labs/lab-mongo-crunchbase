    ...
    rl.question('Type an option: ', (option) => {
      switch(option){
        ...
        case "2":
          ...
          break;
          case "5":
          db.collection('companies').find({founded_year:2004,$and:[{founded_month:{$gte:4}},{founded_month:{$lte:6}}]},{name:1,_id:0,founded_year:1,founded_month:1,founded_day:1}).sort({founded_day:1,founded_month:1}).toArray((error, result) => {
            if (error) {
            console.log(error);
            rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
          } else {
