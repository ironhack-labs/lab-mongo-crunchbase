        ...
        rl.question('Type an option: ', (option) => {
            switch (option) {
                ...
                case "4":
                    ...
                    break;
                case "5":
                    db.collection('companies').find({ founded_year: 2004, $and: [{ founded_month: { $gt: 4 } }, { founded_month: { $lt: 6 } }] }, { name: 1, _id: 0 }).toArray((error, result) => {
                        if (error) {
                            console.log(error);
                            rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                        } else {
                ...
                case "6":
                    ...
                    break;
                case "7":
                    db.collection('companies').find({}, { name: 1, number_of_employees: 1, _id: 0 }).toArray((error, result) => {
                        if (error) {
                            console.log(error);
                            rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                        } else {
                ...
                case "9":
                    ...
                    break;
                case "10":
                    db.collection('companies').find({ name: "Facebook", $all: [{ "products.name": 1 }]}, {products: 1, _id: 0}).toArray((error, result) => {
                    if (error) {
                        console.log(error);
                        rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                    } else {
