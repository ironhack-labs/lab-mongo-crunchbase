      ...
      rl.question('Type an option: ', (option) => {
        switch (option) {
          ...
          case "5":
            ...
            }, { "name": 1,_id: 0 }).sort({ "founded_month": 1, "founded_day": 1 }).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
          ...
          case "7":
            db.collection('companies').find({}, { name: 1, "number_of_employees": 1, _id: 0 }).sort({ "number_of_employees": -1 }).limit(10).toArray((error, result) => {
              if (error) {
                ...
                console.log("\n" + result.map(el =>  el.name + " with " + el.number_of_employees  + " employees").join("\n"));
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              }
            })
            break;
          ...
          case "9":
            db.collection('companies').find({ name: "Facebook" }, { "number_of_employees": 1, _id: 0 }).toArray((error, result) => {
              if (error) {
                ...
                console.log("\n" + result.map(el => el.number_of_employees).join());
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              }
            })
            break;
          case "10":
            db.collection('companies').find({ name: "Facebook" }, { "products.name": 1, _id: 0 }).toArray((error, result) => {
              if (error) {
                ...
                console.log("\n" + result[0].products.map(el => el.name).join("\n"));
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              }
            })
            break;
          case "11":
            db.collection('companies').find({ name: "Facebook", "relationships.is_past": false }, { "relationships": 1, _id: 0 }).toArray((error, result) => {
              if (error) {
                ...
                console.log("\n" + result[0].relationships.map(el => el.person.first_name + " " + el.person.last_name).join("\n"))//.map(el => el.name));
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              }
            })
            break;
          ...
          case "14":
            db.collection('companies').find({ name: "Facebook" }, { "competitions": 1, _id: 0 }).toArray((error, result) => {
              if (error) {
                ...
                console.log("\n" + result[0].competitions.map(el => el.competitor.name).join("\n"));
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              }
            })
            break;
          ...
          case "17":
            db.collection('companies').find({ "offices.city": "London" }, { "name": 1, "offices.city.$": 1, _id: 0 }).toArray((error, result) => {
              if (error) {
                ...
                console.log("\n" + result.map(el => el.name + " located at " + el.offices[0].address1 + " " + el.offices[0].address2 + ", " + el.offices[0].zip_code + " " + el.offices[0].city).join('\n'));
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              }
            })
            break;
          case "18":
            db.collection('companies').find({ $and: [{ "tag_list": /social-networking/ }, { "founded_year": { $gte: 2002 } }, { "founded_year": { $lte: 2016 } }, {"offices.city": "New York"}] }, { "name": 1, _id: 0 }).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
