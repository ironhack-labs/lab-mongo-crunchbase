  ...
  function mainMenu() {
    ...
    rl.question("Type an option: ", option => {
      switch (option) {
        ...
        break;
        case "7":
        db.collection('companies').find({number_of_employees:{$gte:181000}}, {name: 1, _id: 0,number_of_employees:1}).sort({number_of_employees:1}).toArray((error, result) => {
            if (error) {
            console.log(error);
            rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
            } else {
        ...
        db.collection('companies').find({name:'Facebook'}, { 'products.name':1,_id: 0}).toArray((error, result) => {
            ...
            result[0].products.forEach((e)=>{console.log(e.name)});
            rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
            }
        });
        break;
        ...
        case "12":
            ...
            if (error) {
              ...
              var peopleLeft=[];
              result[0].relationships.forEach(function(element){
                if (element.is_past===true){
                  peopleLeft.push(element);
                }
              });
              console.log(peopleLeft.length);
              rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
            }
          })
        break;
        case "13":
        db.collection('companies').find({"relationships.person.permalink":"david-ebersman"},{name: 1, _id: 0}).count((error, result) => {
            if (error) {
            console.log(error);
            rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
            } else {
