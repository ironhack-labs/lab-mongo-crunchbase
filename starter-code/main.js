    ...
    function mainMenu(){
      ...
      break;
					case "4":
						...
						break;
						case "5":
							db.collection('companies').find({"founded_year": 2004, "founded_month": {$gte:4, $lte: 6}}, {name: 1, _id: 0}).sort({founded_month: 1, founded_day: 1}).toArray((error, result) => {
							if (error) {
								console.log(error);
								rl.question(`\nType enter to continue: `, (answer) => { mainMenu();});
							} else {
						...
						break;
						case "7":
							db.collection('companies').find({"number_of_employees" : {$gte: 10}},{name: 1, number_of_employees: 1, _id: 0}).sort({number_of_employees: 1}).limit(10).toArray((error, result) => {
							if (error) {
								console.log(error);
								rl.question(`\nType enter to continue: `, (answer) => { mainMenu();});
							} else {
						...
						break;
						case "10":
							db.collection('companies').find({"products.name": "Facebook" },{products:1, _id: 0}).toArray((error, result) => {
							if (error) {
								console.log(error);
								rl.question(`\nType enter to continue: `, (answer) => { mainMenu();});
							} else {
							...
							if (error) {
								...
								result[0].relationships.forEach(relationships => {
									if (relationships.is_past === true) {
										console.log(`
											name: ${relationships.person.first_name}
											last name: ${relationships.person.last_name}
											`);
									}
								});
								rl.question(`\nType enter to continue: `, (answer) => { mainMenu();});
						...
						case "12":
							...
							if (error) {
								...
								var counter = 0;
									...
									console.log(`${counter} personas ya no trabajan en facebook`);
								rl.question(`\nType enter to continue: `, (answer) => { mainMenu();});
							}
						});
						break;
