        ...
        function mainMenu() {
            ...
            rl.question('Type an option: ', (option) => {
                switch (option) {
                    ...
                    case "5":
                        db.collection('companies').find({ $and: [{ founded_year: 2004 }, { founded_month: { $gte: 4 } }, { founded_month: { $lte: 6 } }] }, { name: 1, founded_month: -1, _id: 0 }).toArray((error, result) => {
                            if (error) {
                                console.log(error);
                                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                            } else {
                    ...
                    case "6":
                        db.collection('companies').find({ "office": "Barcelona" }, { name: 1, _id: 0 }).count((error, result) => {
                            if (error) {
                                console.log(error);
                            } else {
                                console.log("Companies have offices in Barcelona: " + result);
                                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                            }
                        })
                        break;
                    ...
                    case "8":
                        db.collection('companies').find({ "name": "Facebook" }, { name: 1, _id: 0 }).count((error, result) => {
                            if (error) {
                                ...
                                console.log("Company with the name Facebook: " + result);
                                rl.question(`\Type enter to continye : `, (answer) => { mainMenu() });
                            }
                        })
