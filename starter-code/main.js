...
mongoClient.connect(url, (error, db) => {
  if (error) {
    ...
    function mainMenu(){
        ...
        rl.question('Type an option: ', (option) => {
          switch(option){
            ...
            db.collection('companies').find({}, {name: 1, _id: 0}).toArray((error, result) => {
              ...
              break;
            case "2":
            db.collection('companies').find({}, {name: 0, _id: 0}).toArray((error, result) => {
                if (error) {
                  console.log(error);
                  rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                } else {
              ...
              break;
              case "4":
              db.collection('companies').find({"founded_year": 2004, "founded_month":02}, {name: 1, _id: 0}).toArray((error, result) => {
                  if (error) {
                    console.log(error);
                    rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                  } else {
             ...
             break;
             case "5":
             db.collection('companies').find({"founded_year": 2004, "founded_month":4, "founded_month":5, "founded_month":6}, {name: 1, _id: 0}).toArray((error, result) => {
                 if (error) {
                   console.log(error);
                   rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                 } else {
        ...
        break;
        case "10":
        db.collection('companies').find({name: "Facebook"}, {name: 1, products: 1, _id: 0}).sort({number_of_employees: -1}).limit(10).toArray((error, result) => {
            if (error) {
              console.log(error);
              rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
            } else {
       ...
       break;
       case "11":
       db.collection('companies').find({ "$and":[ {name: "Facebook"}, {"relationships.is_past": false}]},  {"relationships.person.first_name": 1, "relationships.person.last_name": 1, "relationships.is_past": 1, _id: 0}).toArray((error, result) => {
           if (error) {
             console.log(error);
             rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
           } else {
      ...
      break;
      case "12":
      db.collection('companies').find({ "$and":[ {name: "Facebook"}, {"relationships.is_past": true}]},  {name: 1, products: 1, _id: 0}).toArray((error, result) => {
          if (error) {
            console.log(error);
            rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
          } else {
