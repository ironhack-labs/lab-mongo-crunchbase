  ...
  function mainMenu(){
    ...
    rl.question('Type an option: ', (option) => {
      switch(option){
        ...
        break;
        case "7":
        db.collection('companies').find({number_of_employees:{$gte:180502}},{name: 1, _id: 0, number_of_employees: 1}).sort({number_of_employees:1}).toArray((error, result) => {
            if (error) {
              console.log(error);
              rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
            } else {
        ...
        break;
        case "10":
        db.collection('companies').find({name:"Facebook"},{"products.name":1, _id: 0, "products.name":1}).toArray((error, result) => {
            if (error) {
              console.log(error);
              rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
            } else {
              result[0].products.forEach((i)=>{console.log(i.name)});
              rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
            }
        });
        break;
        case "11":
        db.collection('companies').find({name:"Facebook"},{relationship:1 ,"relationships.person":1,"relationships.is_past":1, _id: 0}).toArray((error, result) => {
            if (error) {
              console.log(error);
              rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
            } else {
