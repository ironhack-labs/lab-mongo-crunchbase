    ...
    if (error) {
      ...
      function mainMenu(){
        ...
        rl.question('Type an option: ', (option) => {
          ...
          break;
          case "7":
            db.collection('companies').find({"number_of_employees" : {$gt : 0}},{"name" :1, "number_of_employess":1}).sort({"number_of_employess":-1}).limit(10).toArray((error, result) => {
              if (error) {
                  console.log(error);
              }
              else {
          ...
          db.collection('companies').find({ "name": "Facebook"},{"products.name":1,_id:0}).toArray((error, result) => {
              if (error) {
                  ...
                  result[0].products.forEach((products)=>{
