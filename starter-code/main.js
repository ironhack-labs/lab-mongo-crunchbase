      ...
      rl.question('Type an option: ', (option) => {
        switch(option){
          ...
          case "4":
            ...
            break;
          case "5":
            db.collection('companies').find({founded_month: {$gte: 4, $lte: 6 }}, {name: 1, _id: 0}).sort( { "founded_month": 1, "founded_day": 1 } ).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
          ...
          case "13":
            ...
            break;
          case "14":
            db.collection('companies').find({"name": "Facebook"}, {"competitions": 1, name: 1, _id: 0}).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
          ...
          case "15":
            ...
            break;
          case "16":
            db.collection('companies').find({"tag_list": {$regex: /social-networking/}, "founded_year": {$gte: 2002, $lte: 2016}}, {name: 1, _id: 0}).count((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
