    ...
    if (error) {
      ...
      function mainMenu(){
        ...
        rl.question('Type an option: ', (option) => {
          switch(option){
            case "1":
            db.collection('companies').find({}, {name: 1, _id: 0}).toArray((result, error) => {
                if (error) {
                  console.log(error);
                ...
                db.collection('companies').find({ founded_year: 2004 }, { name: 1, _id: 0 }).toArray((error, result) => {
                  ...
                  db.collection('companies').find( {$and: [{ founded_year: 2004, founded_month: { $gte: 4 }, founded_month: { $lte: 6 } }]}, { name: 1, _id: 0 }).sort({ founded_month: 1, founded_day: 1 }).toArray((error, result) => {
                    ...
                    case "6":
                      ...
                      db.collection('companies').find({"number_of_employees": {$gt : 0}}, { name: 1,"number_of_employees": 1}).sort( { "number_of_employees": -1 } ).limit(10).toArray((error, result) => {
                          if (error) {
                              console.log(error);
                              rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                          } else {
                        ...
                        db.collection('companies').find({"name": "Facebook"},{name:1, competitions:1, _id:0}).toArray((error, result) => {
                          if (error) {
                            ...
                            result[0].competitions.forEach ( (competitions) => { console.log(competitions.name); });
                            rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                          }
                        })
                        break;
