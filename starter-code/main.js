    ...
    rl.question('Type an option: ', (option) => {
      switch(option){
        ...
        db.collection('companies').find({founded_year: 2004, founded_month: 2}, {name: 1, _id: 0}).toArray((error, result) => {
          ...
          break; 
        case "5":
        db.collection('companies').find({founded_year: 2004, founded_month: {$gte: 4, $lt: 7}}, {name: 1, _id: 0}).sort({founded_month: -1}).toArray((error, result) => {
            if (error) {
              console.log(error);
              rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
            } else {
          ...
          break;
        case "6":
        db.collection('companies').find({"overview" : /.*Barcelona.*/}, {name: 1, _id: 0}).toArray((error, result) => {
            if (error) {
              console.log(error);
              rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
            } else {
        ...
        db.collection('companies').find({name: "Facebook"}, {number_of_employees: 1}).toArray((error, result) => {
          ...
          break;
        case "10":
        db.collection('companies').find({name: "Facebook"}, {products: }).toArray((error, result) => {
            if (error) {
              console.log(error);
              rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
            } else {
