      ...
      rl.question('Type an option: ', (option) => {
        switch (option) {
          ...
          case "4":
            ...
            break;
          case "5":
            db.collection('companies').find({ $and: [{ founded_month: { $gte: 4 } }, { founded_month: { $lte: 6 } }], founded_year: 2004 }, { name: 1, _id: 0 }).sort({ name: 1 }).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
          ...
          case "6":
            ...
            break;
          case "7":
            db.collection('companies').find({ number_of_employees: { $exists: true } }, { name: 1, number_of_employees: 1, _id: 0 }).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
                console.log(result.sort((a, b) => b.number_of_employees - a.number_of_employees).slice(0, 10));
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              }
            })
            break;
