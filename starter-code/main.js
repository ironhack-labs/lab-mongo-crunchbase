    ...
    function mainMenu(){
      ...
      rl.question('Type an option: ', (option) => {
        switch(option){
          ...
          case "5":
          db.collection('companies').find({ $and: [ {"founded_year": 2004 },{ "founded_month":{$gt:3}} ,{"founded_month":{$lt:7} }]}, {name: 1, _id: 0} ).sort({"founded_month": 1}).toArray((error, result) => {
            if (error) {
              console.log(error);
              rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
            } else {
          ...
          db.collection('companies').find({ "name": "Facebook" }).toArray((error, result) => {
            ...
            case "10":
            db.collection('companies').find( {"name": "Facebook"}, {_id: 0, "products.name": 1} ).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
            ...
            db.collection('companies').find( {"name": "Facebook"}, {_id: 0, "relationships": 1} ).toArray((error, result) => {
              if (error) {
                ...
                let employee = (result[0].relationships).filter((item) => item.is_past == false)
                console.log( employee.map((item) => item.person.first_name + " " + item.person.last_name  ) );
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              }
            });
              break;
              ...
              db.collection('companies').find( {"name": "Facebook"}, {_id: 0, "relationships": 1} ).toArray((error, result) => {
                if (error) {
                  ...
                  let employee = (result[0].relationships).filter((item) => item.is_past)
