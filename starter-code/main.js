        ...
        switch(option){
          ...
          case "7":
            ...
            case "9":
              ...
              break;
            case "10":
              db.collection('companies').find({"name": "Facebook"}, {"products.name":1, "_id":0}).each((error, result) => {
                if (error) {
                  console.log(error);
                  rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                } else {
              ...
              break;
            case "11":
              db.collection('companies').find({"name":"Facebook", "relationships.is_past": true}, {"relationships.person":1, "_id":0}).each((result, error) => {
                if (error) {
                  console.log(error);
                  rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                } else {
          ...
          case "12":
           db.collection('companies').aggregate([
             ...
             { $group: { _id: {"name": "Facebook"}, count: { $sum: 1 } } }
            ...
            break;
         case "13":
           db.collection('companies').find({"relationships.person.first_name": "David", "relationships.person.last_name": "Ebersman"},{"name":1, "_id":0}).toArray((result, error) => {
             if (error) {
               console.log(error);
               rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
             } else {
           ...
           break;
        case "14":
         db.collection('companies').find({"name": "Facebook"}, {"competitions":1, "_id":0}).toArray((result, error) => {
           if (error) {
             console.log(error);
             rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
           } else {
