  ...
  function mainMenu(){
        ...
        rl.question('Type an option: ', (option) => {
          switch(option){
            ...
            case "5":
            //5
            db.collection('companies').find({"founded_year": 2004, $and:[{"founded_month": {$gt: 3}}, {"founded_month": {$lt: 7}}]},{name: 1}).sort({"founded_month": 1}).toArray((error, result) => {
                if (error) {
                  console.log(error);
                  rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
            ...
            case "11":
            //11
            db.collection('companies').find({"name": "Facebook"}, {"relationships.person": 1}).toArray((error, result) => {
                if (error) {
                  console.log(error);
                  rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
