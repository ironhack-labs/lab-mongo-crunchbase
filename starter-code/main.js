          ...
          switch(option){
            ...
            case "2":
              ...
              break;
							case "5":
								db.collection('companies').find({$or:[
									...
									{$and: [{founded_year: 2004},{founded_month: 5}]},
									{$and: [{founded_year: 2004},{founded_month: 6}]}
								]},{name:1,founded_month:1}).sort({"founded_month":1}).toArray((error, result) => {
									if (error) {
										console.log(error);
										rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
									} else {
	              ...
	              break;
							case "6":
								db.collection('companies').find({'offices.city': "Barcelona"}, {name:1}).toArray((error, result) => {
									if (error) {
										console.log(error);
										rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
									} else {
              ...
              break;
					...
					break;
					case "10":
						db.collection('companies').find({name: "Facebook"}, {name:1, products: 1}).toArray((error, result) => {
							if (error) {
								console.log(error);
								rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
							} else {
