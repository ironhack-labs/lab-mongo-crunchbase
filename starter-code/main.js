      ...
      rl.question('Type an option: ', (option) => {
        switch (option) {
          ...
          case "2":
            ...
            break;
          case "3":
            db.collection('companies').find({"founded_year":{$eq: 2004}}).toArray((error,result) => {
              if(error){
                console.log(error);
              }
              else{
          ...
          break;
          case "4":
          db.collection('companies').find({"founded_year": 2004, "founded_month": 2}).toArray((error,result) => {
            if(error){
              console.log(error);
            }
            else{
          ...
          break;
          case "5":
          db.collection('companies').find({$and: [{"founded_month":{$gte :4} }, {"founded_month":{$lte:6}}, {"founded_year" : 2004}]}).sort({"founded_month":1}).toArray((error,result) => {
            if(error){
              console.log(error);
            }
            else{
          ...
          db.collection('companies').find({"name": "Facebook"}, {relationships: 1, _id: 0}).toArray((error,result) => {
            if(error){
              ...
              let counter = 0;
              const totalPast = result[0].relationships;
              for(let i = 0; i< totalPast.length;i++){
                if(!totalPast[i].is_past){
                  counter++;
                }
              }
              console.log(counter);
