      ...
      rl.question('Type an option: ', (option) => {
        switch(option){
          ...
          case "2":
            ...
            break;
          case "3":
            db.collection('companies').find({founded_year: 2004}, {founded_year: 1, _id: 0}).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu();});
              } else {
            ...
            break;
	           ...
	           break;
				  ...
				  case "7":
						...
						break;
					case "8":
						db.collection('companies').findOne({name: "Facebook"}, (error, result) => {
							if (error) {
								console.log(error);
								rl.question(`\nType enter to continue: `, (answer) => { mainMenu();});
							} else {
					...
					case "11":
						...
						break;
					case "12":
						db.collection('companies').find({name: "Facebook", "relationships.is_past": true},{relationships: 1, _id: 0}).toArray((error, result) => {
							if (error) {
								console.log(error);
								rl.question(`\nType enter to continue: `, (answer) => { mainMenu();});
							} else {
