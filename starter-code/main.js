...
const url = `mongodb://localhost:27017/crushbase`
        ...
        switch(option){
          ...
          db.collection('companies').find({}).count((error, result) => {
            if (error) {
              ...
              console.log("The number of companies that were founded on Db: ");
              console.log(result);
              rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
            }
          })
          ...
          case "3":
            db.collection('companies').find({founded_year:2004}).count((error, result) => {                    
              if (error) {
                ...
                console.log("All the companies that were founded on 2004: ");
                console.log(result);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              }
            })
            ...
            db.collection('companies').find({founded_year:2004, founded_month:2}, {name: 1, _id: 0}).toArray((error, result) => {                    
              if (error) {
                ...
                console.log("All the companies that were founded on Feb 2004: ");
                console.log(result);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              }
            })
            ...
            case "5":
              db.collection('company').find({founded_year: 2004, founded_month:{$gte: 4, $lte: 6}}, {name: 1, _id: 0}).sort({"founded_month": 1}).toArray((error, result) => {
                if (error) {
                 console.log(error);
                 rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                  } else {
                    console.log("Companies founded on Summer of 2004");
                     console.log(result);
                      rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                      }
                     })
            ...
            db.collection('companies').find({offices:{city:"Barcelona"}},{name: 1, _id: 0}).toArray((error, result) => {                    
              if (error) {
                ...
                console.log("Companies from Barcelona");
                console.log(result);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              }
            })
...
11.- List the people that are working at Facebook right now (check relationships field) STOPED HERE
