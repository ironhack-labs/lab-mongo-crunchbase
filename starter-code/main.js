    ...
    function mainMenu(){
      ...
      rl.question('Type an option: ', (option) => {
        switch(option){
          ...
          case "5":
          db.collection('companies').find({$and: [{founded_month:{ $gte:4 }},{founded_month: {$lte: 6}}, {founded_year: 2004}]},{name:1, _id: 0}).sort({founded_month: 1}).toArray((error, result) => {
            if (error) {
              console.log(error);
              rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
            }
	          ...
	          db.collection('companies').find({name: "Facebook"}).toArray((error, result) => {
	            if (error) {
	              ...
	              rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
	                ...
	                result[0].relationships.filter((elem) => !elem.is_past).forEach((elem)=> console.log(elem.person.first_name, elem.person.last_name));
		          ...
		          db.collection('companies').find({name: "Facebook"}).toArray((error, result) => {
		            if (error) {
		              ...
		              rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
		                ...
		                console.log(result[0].relationships.filter((elem) => elem.is_past).length);
		              rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
		            }
		          })
		          break;
								...
								// 14.- List by name the competitors of Facebook
									db.collection('companies').find({name: "Facebook"}).toArray((error, result) => {
										if (error) {
											...
											rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
												...
												result[0].competitions.forEach((elem)=> console.log(elem.competitor.name));
											rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
										}
									})
									break;
									...
									// 15.- Names of the companies that has "social-networking" in tag-list (be aware that the value of field is a string check regex operators)
										...
										// 16.- How many companies that has "social-network" in tag-list and founded between 2002 and 2016 inclusive
											...
											db.collection('companies').count({$and: [{"tag_list": {$regex: /social\-networking/}}, {founded_year:{ $gte:2002 }},{founded_year: {$lte: 2016}}]}, ((error, result) => {
												if (error) {
													console.log(error);
													rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
												} else {
