...
const url = `mongodb://localhost:27017/lab-mongo-crunchbase`
        ...
        function mainMenu() {
            ...
            rl.question('Type an option: ', (option) => {
                switch (option) {
                    ...
                    //     db.collection('companies').find({ $and[{ founded_year: 2004 }, { founded_month: $and[{ $gte: 4 }, { $lte: 6 }] }]}, { name: 1, _id: 0 }).toArray((error, result) => {
                    //         if (error) {
                    //             console.log(error);
                    //             rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                    //         } else {
                    ...
                    case "10":
                        ...
                        break;
                    case "11": // POR QUÉ NO FUNCIONAAAAAAA AAAAAAA AAAAAA AAAAAA
                        ...
                        db.collection('companies').find({ 'name': 'Facebook' }, { 'relationships': 1 }).toArray((error, result) => {
                            if (error) {
                                ...
                                result.forEach((elem) => {
                                    if (elem.is_past === false) {
                                        console.log(elem.person.first_name)
                                    }
                                })
