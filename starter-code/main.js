            ...
            break;
	          ...
	          db.collection('companies').find({"name":"Facebook"},{"products.name":1}).toArray((error, result) => {
	            ...
	            rl.question(`\nType enter to continue: `, (answer) => { mainMenu(); });
						...
						break;
						case "11":
	          db.collection('companies').find({"name":"Facebook"},{"relationships.is_past":"false"},{"relationships.person.first_name":1}).toArray((error, result) => {
	            if (error) {
	            console.log(error);
	            rl.question(`\nType enter to continue: `, (answer) => { mainMenu(); });
	          } else {
	            result[0].relationships.forEach(relationships => console.log("relationships.person.first_name"));
	            rl.question(`\nType enter to continue: `, (answer) => { mainMenu(); });
	          }
	          });
						break;
