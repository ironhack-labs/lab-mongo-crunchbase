      ...
      function mainMenu(){
        ...
        rl.question('Type an option: ', (option) => {
          switch(option){
            ...
            break;
            case "2":
                db.collection('companies').distinct("name", function(error, result) {
                    if (error) {
                        console.log(error);
                        rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                    } else {
            ...
            break;
            case "5":
                db.collection('companies').find({ $and:[{founded_year: 2004}, { $or: [{founded_month: 4},{founded_month: 5},{founded_month: 5}]}]}).toArray((error, result) => {
                    if (error) {
                        console.log(error);
                        rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                    } else {
