      ...
      rl.question('Type an option: ', (option) => {
        switch(option){
          ...
          case "4":
            ...
            break
          case "5":
            db.collection('companies').find({"founded_month": {$gte:4, $lte:6}}, {"name":1, "_id":0, "founded_month":1}).sort({"founded_month": 1}).toArray((error, result) => {
              if (error) {
                console.log(error)
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() })
              } else {
          ...
          case "6":
            ...
            break
          case "7":
            db.collection('companies').find({}, {"number_of_employees": 1, "_id":0}).sort({"number_of_employees":-1}).limit(10).toArray((error, result) => {
              if (error) {
                console.log(error)
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() })
              } else {
