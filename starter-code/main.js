    ...
    function mainMenu(){
      ...
      rl.question('Type an option: ', (option) => {
        switch(option){
          ...
          break;
          case "5":
            db.collection('companies').find( {$or:[
              {"founded_year": 2004, "founded_month": 4},
              {"founded_year": 2004, "founded_month": 5},
              {"founded_year": 2004, "founded_month": 6}
            ]},{name: 1, founded_month:1,_id: 0}).sort({"founded_month":1}).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
          ...
          break;
          case "12":
            db.collection('companies').find({name: "Facebook","relationships.is_past": true},{"relationships.person.first_name":1,_id: 0}).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
