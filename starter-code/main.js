      ...
      rl.question('Type an option: ', (option) => {
        switch(option){
          ...
          case "3":   // 3.- How many companies were founded in 2004?
            db.collection('companies').find({founded_year:{$gte:2004}}).count((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
          ...
          case "5":   // 5.- List by name all companies founded in the summer of 2004 (april to june) sorted by date.
            db.collection('companies').find({founded_year:2004, $and:[{founded_month:{$gte:4}}, {founded_month:{$lte:6}}] } ,{name:1, _id:0}).sort({founded_month:1, founded_day:1}).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
          ...
          case "10":   // 10.- List the name of all the products of Facebook
            db.collection('companies').find({"name":"Facebook"}, {"products.name":1, _id:0}).toArray((error, result) => {
              if (error) {
                ...
                console.log(result[0].products.map(el=>el.name));
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              }
            })
            break;
          ...
          case "14":   // 14.- List by name the competitors of Facebook
            db.collection('companies').find({name:"Facebook"},{"competitions.competitor.name":1, _id:0}).toArray((error, result) => {
              if (error) {
                ...
                console.log(result[0].competitions.map(el=>el.competitor.name));
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              }
            })
            break;       
          ...
          case "16":   // 16.- How many companies that has "social-network" in tag-list and founded between 2002 and 2016 inclusive
            db.collection('companies').find({tag_list:/social-networking/, $and:[{founded_year:{$gte:2002}},{founded_year:{$lte:2016}}]}).count((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
         ...
         case "17":   // 17.- Names and locations of companies that have offices in London
            db.collection('companies').find({offices:{$elemMatch:{city:"London"}}}, {name:1, "offices.city":1, offices:{$elemMatch:{"city":"London"}}, _id:0}).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
                console.log(result.map(el=>"Company: "+el.name+" Address:"+el.offices.reduce((p,e)=>e.address1+", "+p, "")));
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              }
            })
            break;               
