          ...
          rl.question('Type an option: ', (option) => {
            switch(option){
              ...
              case "4":
                ...
                break;
                //    5.- List by name all companies founded in the summer of 2004 (april to june) sorted by date.//day?
              case "5":
                db.collection('companies').find({'founded_year': 2004, 'founded_month': {$in: [ 4,5,6 ] }} ,
                 {name: 1, _id: 0}).sort({"founded_date": 1}).toArray((error, result) => {
                  if (error) {
                    console.log(error);
                    rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                  } else {
              ...
              case "7":
                db.collection('companies').find({} , {name: 1, _id: 0, 'number_of_employees': 1})
                ...
                break;
              case "8":
                db.collection('companies').find({name: "Facebook"} , {name: 1, _id: 0})
                ...
                break;
              case "9":
                db.collection('companies').find({name: "Facebook"} , {name: 1, _id: 0, 'number_of_employees': 1})
                  .toArray((error, result) => {
                  if (error) {
                    console.log(error);
                    rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
...
//List the name of all the products of Facebook
              case "10":
                db.collection('companies').find({name: "Facebook"} , {name: 1, _id: 0, 'products.name': 1})
                  .toArray((error, result) => {
                  if (error) {
                    console.log(error);
                    rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
...
// List the people that are working at Facebook right now (check relationships field)
              case "11":
                db.collection('companies').find({name: "Facebook", "relationships.is_past": false} , {name: 1, _id: 0, 'relationships.person': 1})
                  .toArray((error, result) => {
                  if (error) {
                    console.log(error);
                    rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
