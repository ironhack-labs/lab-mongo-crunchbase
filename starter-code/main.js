...
const url = `mongodb://localhost:27017/crunchdatabase`
...
function mainMenu(){
      ...
      rl.question('Type an option: ', (option) => {
        switch(option){
          ...
          break;
          case "5":
           db.collection('companies').find({ $and: [{ founded_year : 2004 }, { founded_month: { $gte: 4 }}, { founded_month : { $lte: 7 }} ]}, {name: 1, _id: 0}).sort( { "name": 1 } ).toArray((error, result) => {
                if (error) {
                    console.log(error);
                    rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                } else {
          ...
          break;
          case "6":
            db.collection('companies').find({ "offices.city" : "Barcelona" }, {name: 1, _id: 0}).sort( { "name": 1 } ).toArray((error, result) => {
                if (error) {
                    console.log(error);
                    rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                } else {
          ...
          break;
           case "7":
            db.collection('companies').find({ number_of_employees: { $ne: null } }, {name: 1, number_of_employees: 1, _id: 0}).sort( { "number_of_employees": -1 } ).limit(10).toArray((error, result) => {
                if (error) {
                    console.log(error);
                    rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                } else {
          ...
          break;
          case "10":
           db.collection('companies').find({ name: "Facebook" }, {name:1, "products.name": 1, _id: 0}).forEach((error, result) => {
                if (error) {
                    console.log(error);
                    rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                } else {
