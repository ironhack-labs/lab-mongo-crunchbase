        ...
        switch(option){
          ...
          db.collection('companies').count({"founded_year": 2004}, (error, result) => {
            ...
            db.collection('companies').find({"founded_year": 2004, "founded_month": 2}, {name:1, _id:0}).toArray((error, result) => {
              ...
              break;
              case "5":
              db.collection('companies').find({"founded_year": 2004, "founded_month": {$lte: 6}, "founded_month": {$gte:4 }}, {name:1, _id: 0}).sort({"founded_day": 1}).toArray((error, result) => {
                ...
                db.collection('companies').find({"name": "Facebook"}, {name: 1, number_of_employees: 1, _id: 0}).toArray((error, result) => {
                  ...
                  db.collection('companies').find({"name": "Facebook"}, {"relationships": 1, _id: 0}).toArray((error, result) => {
                    ...
                    if (error) {
                      ...
                      result[0].relationships.forEach((element) => (console.log(element.name)))
                    ...
                    break;
                    case "12":
                  db.collection('companies').count({"relationships.is_past": true}, (error, result) => {
                  ...
                  db.collection('companies').find({"relationships.person.permalink": "david-ebersman"}, {"name": 1, _id: 0}).toArray((error, result) => {
                    ...
                    db.collection('companies').find({"name": "Facebook"}, {"competitions": 1, _id: 0}).toArray((error, result) => {
                      ...
                      if (error) {
                        ...
                        console.log(result[0].competitions.forEach((element) => (console.log(element.name))))
                      ...
                      break;
                      case "15":
                      db.collection('companies').find({"tag_list": {$regex: ".*social-networking.", $options: "i"}}, {name: 1, _id: 0}).toArray((error, result) => {
                        ...
                        break;
                        case "16":
                        db.collection('companies').count({$and:[{"tag_list": {$regex: ".*social-networking.", $options: "i"}}, {"founded_year": {$gte: 2002}}, {"founded_year": {$lte: 2016}}]}, (error,result) => {
                        ...
                        db.collection('companies').find({"offices.city": "London"}, {name:1, offices: 1, _id: 0}).toArray((error, result) => {
                          ...
                          if (error) {
                            ...
                            console.log(result)
                            result.forEach((element) => console.log(element.name, element.offices.forEach((element2)=>console.log(element2.city))))
                          ...
                          break;
                          case "18":
                        db.collection('companies').count({$and:[{"tag_list": {$regex: ".*social-networking.", $options: "i"}}, {"founded_year": {$gte: 2002}}, {"founded_year": {$lte: 2016}}, {"offices.city" : "New York"}]}, (error,result) => {
