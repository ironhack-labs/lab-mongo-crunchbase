        ...
        switch(option){
          ...
          case "3":
            db.collection('companies').find({"founded_year":2004}).count((error, result) => {
              ...
              break;
            case "4":
              db.collection('companies').find({"founded_month" : 2}, {name: 1, _id: 0}).toArray((error, result) => {
                if (error) {
                  console.log(error);
                  rl.question(`\nType enter to continue: `, (answer) => { mainMenu(); });
                } else {
          ...
          case "10":
            db.collection('companies').find({"name":"Facebook"}, {"products.name": 1, _id: 0}).toArray((error, result) => {
              if (error) {
                ...
                result = result[0].products.forEach( (e) => console.log(e.name));
                console.log(result);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu(); });
              }
            });
          ...
          case "11":
            db.collection('companies').find({"name":"Facebook"}, {"relationships": 1, _id: 0}).toArray((error, result) => {
              if (error) {
                ...
                result = result[0].relationships.filter( (e) => !e.is_past ).forEach( (e) => console.log(e.person.first_name));
                console.log(result);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu(); });
              }
            });
