        ...
        rl.question('Type an option: ', (option) => {
          switch(option){
            case "1":
              ...
              break;
            case "2":
                db.collection('companies').count( {}, function(err, result){
                    if (error) {
                      console.log(error);
                      rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                    } else {
              ...
              break;
                case "3":
                db.collection('companies').count( {founded_year: 2004}, function(err, result){
                    if (error) {
                      console.log(error);
                      rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                    } else {
              ...
              break;
              case "14":
                db.collection('companies').find({$and:[{name : "Facebook"}]},{ name: 1,competitions:1, _id: 0}).toArray((error, result) => {
                    if (error) {
                      console.log(error);
                      rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                    } else {
              ...
              break;
              case "17":
                db.collection('companies').find( { "offices.city" : "London"} ,{name: 1 ,_id: 0}).toArray((error, result) =>{ //MAAAAAL
                    if (error) {
                      console.log(error);
                      rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                    } else {
