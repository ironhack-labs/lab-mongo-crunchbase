          ...
          switch(option){
            ...
            case "11":
                db.collection('companies').find({name: "Facebook"}, {relationships: 1, _id: 0}).toArray((error, result) => {
                    ...
                    result[0].relationships.forEach(elm => elm.is_past === false ? console.log(elm.person.permalink) : null)
                    rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                    }
                })
            break;
            case "12":
                db.collection('companies').find({name: "Facebook"}, {relationships: 1, _id: 0}).toArray((error, result) => {
                    ...
                    result[0].relationships.forEach(elm => {
                        elm.is_past === true ? arr.push(elm) : null
                    })
            ...
            case "13":
                db.collection('companies').find({"relationships.person.permalink": "david-ebersman"}, {name: 1, _id: 0}).toArray((error, result) => {
                    ...
                    result.forEach(elm => console.log(elm.name))
                    rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                    }
                })
            break;
            ...
            break;
            case "16":
                db.collection('companies').find({$and: [{tag_list: {$regex: "social-network,"}}, {founded_year: {$gte: 2002}}, {founded_year: {$lte: 2016}}]}, {name: 1, _id: 0}).toArray((error, result) => {
                    if (error) {
                    console.log(error);
                    rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                    } else {
