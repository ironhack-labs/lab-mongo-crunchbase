        ...
        rl.question('Type an option: ', (option) => {
          switch(option){
            case "1":
              ...
              break;
            case "2":
                db.collection('companies').find({}, {name: 1, _id: 0}).count((error, count) => {
                    if (error) {
                      console.log(error);
                      rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                    } else {
              ...
              break;
              case "3":
                db.collection('companies').find({founded_year: 2004}).count((error, count) => {
                    if (error) {
                      console.log(error);
                      rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                    } else {
                  ...
                  case "4":
                    db.collection('companies').find({founded_year: 2004, founded_month: {$gte: 4, $lt: 7}}, {name: 1, _id: 0}).toArray((error, result) => {
                      if (error) {
                        console.log(error);
                        rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                      } else {
                  ...
                  case "5":
                    ...
                    db.collection('companies').find({'offices.city':"Barcelona"}).toArray((error, result) => {
                      ...
                      case "7":
                    db.collection('companies').find({},{name: 1, number_of_employees: 1, _id: 0}).sort({
                      number_of_employees: -1}).limit(10).toArray((error, result) => {
                        if (error) {
                          console.log(error);
                          rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                        } else {
                    ...
                    //     })
                    //   case "12":
                    //     db.collection('companies').find({name: 'Facebook'}, {'relationships.person': 1,'relationships.is_past': 0, _id: 0}).toArray((error, result) => {
                    //         if (error) {
                    //           console.log(error);
                    //           rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                    //         } else {
                    //           result[0].relationships.forEach((person) => {
                    //             console.log(person)});
                    //           rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                    //         }
                    //       })
