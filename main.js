    ...
    function mainMenu(){
        ...
        rl.question('Type an option: ', (option) => {
          switch(option){
            ...
            case "4":
                    db.collection('companies').find({$and:[{founded_year: { $eq: 2004 }},{founded_month:{$eq:2}}]}, {name: 1, _id: 0}).toArray((error, result) => {
                        if (error) {
                          console.log(error);
                          rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                        } else {
            ...
            case "5":
                    db.collection('companies').find({$and:[{founded_year: { $eq: 2004 }},{founded_month:{$lte:6}},{founded_month:{$gte:4}}]}, 
                                                    {name: 1, _id: 0},
                                                    {$sort:{day:1 ,month:1}}).toArray((error, result) => {
                        if (error) {
                          console.log(error);
                          rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                        } else {
                         ...
                         break;
             case "6":
                    db.collection('companies').find({"offices.city":"Barcelona"},{name: 1,founded_year:1, _id: 0}).toArray((error, result) => {
                        if (error) {
                          console.log(error);
                          rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                        } else {
            ...
            case "10":
                    db.collection('companies').find({"name":"Facebook"},{"products.name":1,_id:0}).toArray((error, result) => {
                        if (error) {
                         ...
                         break;       
                        //  find({$and:[{founded_year: { $eq: 2004 }},{founded_month:{$eq:2}}]}
            case "11":
                    db.collection('companies').find({$and:[{"name":"Facebook"},{relationships:"is_past":true}]},{relationships:1,_id:0}).toArray((error, result) => {
                        if (error) {
                          console.log(error);
                          rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                        } else {
