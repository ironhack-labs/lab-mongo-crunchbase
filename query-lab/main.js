            ...
            case "4":
              db.collection('companies').find({ $and: [{ founded_year: 2004 }, { founded_month: 2 }]} , {name: 1, _id: 0}).toArray((error, result) => {
                if (error) {
                  ...
                  console.log(result.forEach(elem => console.log(elem.name)));
                  rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                }
            })
              break;
                ...
                case "7":
                  db.collection('companies').find({}, {name: 1, number_of_employees: 1, _id: 0}).sort({number_of_employees: -1}).limit(10).toArray((error, result) => {
                    if (error) {
                      ...
                      console.log(result.forEach(elem => console.log(elem)));
                      rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                    }
                  })
                  break;
              case "8":
                db.collection('companies').find({name: "Facebook"},{name: 1, homepage_url: 1, founded_year: 1, _id: 0}).toArray((error, result) => {
                  if (error) {
                    console.log(error);
                    rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                    } else {
                ...
                break;
                case "9":
                  db.collection('companies').find({name: "Facebook"},{name: 1, number_of_employees}).toArray((error, result) => {
                    if (error) {
                      console.log(error);
                      rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                      } else {
